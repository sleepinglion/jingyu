...
나이들고 음주와 과식을 하면서 배가 나오기 시작해서 지금은 임신상태의 배이긴 하지만
             아들 친구라는 놈이 놀러 왔는데 배가 금방이라도 터질것 같다.
나는 저 나이때는 저렇지 않았는데 젊은나이에 운동과 관리가 저리 안 될 수도 있나 싶다.
물어보니 탄산음료 먹고 운동을 하지 않아 저렇게 처참하게 된듯하다.  역시 탄산음료는 입에도 대지 말아야할 최악의 음료이다.
')

Blog.create!(id: 24, user_id: 1, blog_category_id: 3,title: '삼인행스승',description: '세명이 길을가면 그중에 스승으로 삼을만한 사람이 있다.', created_at: 1.day.ago,tag_list: '',
             content: '
')

Blog.create!(id: 25, user_id: 1, blog_category_id: 3,title: '위대한 정충신장군',description: '노비에서 장군이되어 나라를 구한 장군이야기 들어보셨나요?', created_at: 1.day.ago,tag_list: '',
             content: '
')
